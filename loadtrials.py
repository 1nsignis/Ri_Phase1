...
def load_trials(eval : trec.TrecEvaluation) -> tuple[ list[Trial], list[str] ]:

    if path.exists("doc_ids.bin") and path.exists("full_documents.bin"):
        return pickle.load( open( "doc_ids.bin", "rb" ) ), pickle.load( open( "full_documents.bin", "rb" ) )    
